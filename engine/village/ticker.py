...
from .heuristics import heuristic_decision, in_world_reason
...
# Conversation turn bands (LLM-generated utterances only). A conversation runs
# alternating turns until the model signals it has run its course (wrapUp) or a
# familiarity/sentiment-scaled ceiling is hit — so acquaintances chat briefly
# while close, warm pairs talk at length (research: minTurns/maxTurns band +
# model-side natural-stop gate for deep, flowing dialogue).
CONVO_MIN_TURNS = 4          # floor so an exchange has a real back-and-forth
...
class Ticker:
    ...
    def _run_conversations(self, sim_iso: str) -> int:
        ...
        conversation gracefully (Req 10.8). Utterance text is ALWAYS
        LLM-generated by the harness — there is no local/mock fallback, so when
        no runtime is configured (or the budget is exhausted) no conversation is
        produced this tick.
        """
        # No runtime => no LLM => no conversation. We never fabricate utterances.
        if self.runtime is None:
            return 0
        ...
        for agent in self.world.agents.values():
            ...
            # Scale the conversation length to the initiator<->partner
            # relationship: close, warm pairs talk longer; near-strangers keep
            # it short. The model may also end early via a wrapUp signal.
            min_turns, max_turns = self._conversation_turn_band(convo)

            # Mutable per-conversation state shared with the provider closure so
            # a model wrapUp signal can end the exchange after one closing turn.
            convo_state = {"topic": None, "wrap_after": None}

            def utterance_provider(conversation, speaker_id, _self=self,
                                   _sim=sim_iso, _min_turns=min_turns,
                                   _max_turns=max_turns, _st=convo_state):
                turn_index = len(conversation.utterances)
                # Honour a prior wrapUp: allow exactly one closing turn, then end.
                if _st["wrap_after"] is not None and turn_index > _st["wrap_after"]:
                    return None
                ...
                # Participants enriched with names + relationship state so the
                # harness can render tone/history as prose (deeper dialogue).
                participants_info = []
                for pid in conversation.participants:
                    other = _self.world.agents.get(pid)
                    rel = _self.social.get_relationship(speaker_id, pid) \
                        if pid != speaker_id else None
                    info = {"id": pid,
                            "name": other.persona.name if other else pid}
                    if rel is not None:
                        info["familiarity"] = rel.familiarity
                        info["sentiment"] = rel.sentiment
                ...
                payload = {
                    ...
                    "persona": persona.persona.to_dict(),
                    "longTermMemory": _self._stm_lines(speaker_id),
                }
                ...
                if not (isinstance(text, str) and text.strip()):
                    return None
                # Capture the opening line as the shared topic recap so later
                # turns stay on one thread instead of drifting.
                if _st["topic"] is None and turn_index == 0:
                    _st["topic"] = text.strip()[:160]
                # A model-side natural stop: only honoured once the exchange has
                # met its floor, and it still permits one graceful closing turn.
                if bool(resp.get("wrapUp")) and turn_index + 1 >= _min_turns \
                        and _st["wrap_after"] is None:
                    _st["wrap_after"] = turn_index + 1
                return text

            try:
                self.social.run_conversation(convo, utterance_provider,
                                             max_utterances=max_turns)
            except Exception:
    ...
    def _conversation_turn_band(self, convo) -> Tuple[int, int]:
        """Pick (min_turns, max_turns) for a conversation from the strongest
        pairwise relationship among its participants.

        Close, warm pairs sustain longer exchanges; near-strangers keep it
        short. Bounded by the module turn constants and the Social_Engine's hard
        MAX_UTTERANCES ceiling.
        """
        best_fam = 0
        best_sent = 0
        parts = convo.participants
        for i in range(len(parts)):
            for j in range(len(parts)):
                if i == j:
                    continue
                rel = self.social.get_relationship(parts[i], parts[j])
                if rel.familiarity > best_fam:
                    best_fam = rel.familiarity
                if rel.sentiment > best_sent:
                    best_sent = rel.sentiment
        if best_fam >= 40 or best_sent >= 40:
            max_turns = CONVO_MAX_TURNS_CLOSE
            min_turns = CONVO_MIN_TURNS
        elif best_fam < 15 and best_sent < 10:
            # Two near-strangers: a brief, polite exchange.
            max_turns = CONVO_MIN_TURNS + 1
            min_turns = CONVO_MIN_TURNS_STRANGER
        else:
            max_turns = CONVO_MAX_TURNS_BASE
            min_turns = CONVO_MIN_TURNS
