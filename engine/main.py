...
def main() -> None:
    ...
    # Conversations and utterances are LLM-generated by the harness (AgentCore
    # Runtime); there is no local/mock utterance fallback. The harness is used
    # whenever a runtime ARN is configured. Set HARNESS_ENABLED=0 only to force
    # it off (e.g. a diagnostic run with no LLM) — decisions then fall back to
    # the deterministic heuristic engine and no conversations are produced.
    harness_enabled = os.environ.get("HARNESS_ENABLED", "1") == "1"
    runtime = (BedrockAgentRuntimeClient(runtime_arn, region, budget)
               if (runtime_arn and harness_enabled) else None)
    if runtime is None:
        print("[engine] no agent runtime configured "
              f"(HARNESS_ENABLED={os.environ.get('HARNESS_ENABLED','1')}, "
              f"arn={'set' if runtime_arn else 'unset'}); decisions use the "
              "deterministic heuristic engine and conversations are disabled",
              flush=True)
